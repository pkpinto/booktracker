...
from ..dependencies.mongodb import Asset


router = APIRouter(prefix='/assets', tags=['Local asset data'])


@router.get('/', summary='List locally stored assets')
async def get_assets(db: Asset = Depends(Asset)):
    return db.list_assets()


@router.get('/{assetid}', summary='Asset details')
async def get_asset(assetid: str, db: Asset = Depends(Asset)):
...
async def upsert_asset_artwork(assetid: str, file: bytes = File(...), db: Asset = Depends(Asset)):
...
async def delete_asset_artwork(assetid: str, db: Asset = Depends(Asset)):
